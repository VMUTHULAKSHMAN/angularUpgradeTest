...
export function getEnabledMockApiServices() {
  if (mockApiConfig.enableMockAPI) {
    if (mockApiConfig.disableMockApiFor) {
      return mockApiServices.filter(value => !mockApiConfig.disableMockApiFor.includes(value.name));
    }
    return mockApiServices;
  }
  return [];
}
